"""Reference RNS negacyclic ring `Z[X]/(X^d + 1)`, ported from lattigo v6
...
# Primality and the array contract live in this package's shared modules.
...
def _prime_factors(n: int) -> set[int]:
    ...
    `_primitive_root` (see module docstring)."""
...
def _primitive_root(q: int, factors: set[int]) -> int:
...
def _generate_ntt_table(q: int, d: int) -> tuple[list[int], list[int], int]:
    ...
    for j in range(1, half):
        idx_prev = _bit_reverse(j - 1, log_nth_root)
        idx_next = _bit_reverse(j, log_nth_root)
