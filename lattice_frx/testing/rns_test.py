...
class RnsTest(parameterized.TestCase):

    def test_crt_roundtrip_including_negatives(self):
        q_moduli = (97, 89)
        Q = 97 * 89
    ...
    def test_crt_roundtrip_three_limbs(self):
        q_moduli = (97, 89, 83)
        Q = 97 * 89 * 83
        bound = Q // 2
        rnd = random.Random(7)
    ...
    def test_centering_boundary_at_exactly_q_half_maps_to_negative(self):
        ...
        Q = 97 * 89
        q_half = Q >> 1
        self.assertEqual(q_half, 4316)

        coeffs = rns.set_big_coeffs([q_half], q_moduli)
        got = rns.reconstruct_centered(coeffs, q_moduli)

        self.assertEqual(got, [q_half - Q])

    def test_fast_path_agrees_for_small_values(self):
    ...
    def test_slow_path_forced_for_large_values(self):
        ...
        self.assertTrue(all(abs(v) < Q // 2 for v in vals))
    ...
    def test_rsh_floor_on_negatives(self):
        self.assertEqual(rns.rsh_floor([-5], 1), [-3])
        self.assertEqual(rns.rsh_floor([-4], 1), [-2])
        self.assertEqual(rns.rsh_floor([-1], 1), [-1])
        self.assertEqual(rns.rsh_floor([5], 1), [2])
        self.assertEqual(rns.rsh_floor([0], 3), [0])

    def test_set_big_coeffs_returns_uint64(self):
        q_moduli = (97, 89, 83)
        coeffs = rns.set_big_coeffs([5, -5, 40], q_moduli)
        self.assertEqual(coeffs.dtype, np.uint64)

    def test_reconstruct_centered_accepts_uint64_and_returns_python_ints(self):
    ...
    def test_lift_centered_returns_uint64(self):
        from_moduli = (97, 89)
        to_moduli = (67, 71, 73)
        coeffs = rns.set_big_coeffs([1, -1, 100], from_moduli)
        lifted = rns.lift_centered(coeffs, from_moduli, to_moduli)
        self.assertEqual(lifted.dtype, np.uint64)

    def test_noncanonical_uint64_input_rejected(self):
    ...
    @parameterized.parameters(np.float64, object)
    def test_non_uint64_dtype_rejected(self, bad_dtype):
    ...
    def test_rescale_floor_matches_three_step_composition(self):
    ...
    def test_lift_centered_round_trip(self):
