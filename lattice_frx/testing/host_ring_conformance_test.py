...
parameterized matrix of op x backend x property.
"""
import random
...
def _make_ring(backend):
    return BACKENDS[backend](Q_MODULI, D)
...
class HostRingConformanceTest(parameterized.TestCase):

    def _assert_no_mutation(self, op_name, fn, *args):
        ...
        for arr, before in snapshots:
            self.assertTrue(
                ...
                f"{op_name} mutated an input array in place",
            )

    # --- (a) dtype/shape per the documented contract ------------------------

    @parameterized.parameters(*((backend,) for backend in sorted(BACKENDS)))
    def test_ops_respect_documented_dtype_and_shape_contract(self, backend):
        ring = _make_ring(backend)
        limbs = len(Q_MODULI)
        a = ring.from_signed(_rand_vec(1))
        b = ring.from_signed(_rand_vec(2))
        acc = ring.from_signed(_rand_vec(3))
        self.assertTrue(a.dtype == np.uint64 and a.shape == (limbs, D))
        ...
        for name, out in outputs.items():
            self.assertEqual(out.dtype, np.uint64, f"{name}: expected uint64, got {out.dtype}")
            self.assertEqual(out.shape, (limbs, D), f"{name}: expected shape {(limbs, D)}, got {out.shape}")
        ...
        self.assertEqual(balanced.dtype, np.int64)
        self.assertEqual(balanced.shape, (D,))
        ...
        self.assertTrue(isinstance(reconstructed, list) and len(reconstructed) == D)
        self.assertTrue(all(isinstance(v, int) for v in reconstructed))

        set_back = rns.set_big_coeffs(reconstructed, Q_MODULI)
        self.assertTrue(set_back.dtype == np.uint64 and set_back.shape == (limbs, D))

        lifted = rns.lift_centered(coeffs, Q_MODULI, Q_OUT_MODULI)
        self.assertTrue(lifted.dtype == np.uint64 and lifted.shape == (len(Q_OUT_MODULI), D))

        rescaled = rns.rescale_floor(coeffs, Q_MODULI, 3, Q_OUT_MODULI)
        self.assertTrue(rescaled.dtype == np.uint64 and rescaled.shape == (len(Q_OUT_MODULI), D))

    # --- (b) purity -- no op mutates its inputs ------------------------------

    @parameterized.parameters(*((backend,) for backend in sorted(BACKENDS)))
    def test_ops_do_not_mutate_their_inputs(self, backend):
        ring = _make_ring(backend)
        ...
        self._assert_no_mutation("ntt", ring.ntt, a)
        self._assert_no_mutation("intt", ring.intt, a_ntt)
        self._assert_no_mutation("add", ring.add, a, b)
        self._assert_no_mutation("sub", ring.sub, a, b)
        self._assert_no_mutation("neg", ring.neg, a)
        self._assert_no_mutation("mul", ring.mul, a_ntt, b_ntt)
        self._assert_no_mutation("mul_add", ring.mul_add, a_ntt, b_ntt, acc_ntt)
        self._assert_no_mutation("mul_scalar", ring.mul_scalar, a, 7)
        self._assert_no_mutation("mul_scalar_then_sub", ring.mul_scalar_then_sub, a, 7, acc)
        self._assert_no_mutation("to_balanced_limb0", ring.to_balanced_limb0, a)

        coeffs = ring.from_signed(_rand_vec(14))
        self._assert_no_mutation("rns.reconstruct_centered", rns.reconstruct_centered, coeffs, Q_MODULI)
        self._assert_no_mutation("rns.lift_centered", rns.lift_centered, coeffs, Q_MODULI, Q_OUT_MODULI)
        self._assert_no_mutation("rns.rescale_floor", rns.rescale_floor, coeffs, Q_MODULI, 3, Q_OUT_MODULI)

    # --- (c) canonicality -- every residue < q_l for its limb ---------------

    @parameterized.parameters(*((backend,) for backend in sorted(BACKENDS)))
    def test_uint64_outputs_are_canonical_per_limb(self, backend):
        ring = _make_ring(backend)
        ...
        for out in outputs:
            self.assertTrue((out < q_col).all())

        coeffs = ring.from_signed(_rand_vec(24))
        reconstructed = rns.reconstruct_centered(coeffs, Q_MODULI)
        self.assertTrue((rns.set_big_coeffs(reconstructed, Q_MODULI) < q_col).all())

        q_out_col = np.array(Q_OUT_MODULI, dtype=np.uint64)[:, None]
        self.assertTrue((rns.lift_centered(coeffs, Q_MODULI, Q_OUT_MODULI) < q_out_col).all())
        self.assertTrue((rns.rescale_floor(coeffs, Q_MODULI, 3, Q_OUT_MODULI) < q_out_col).all())

    # --- (d) algebraic sanity, expectations derived independently -----------

    @parameterized.parameters(*((backend,) for backend in sorted(BACKENDS)))
    def test_algebraic_sanity_derived_independently_of_backend(self, backend):
        ring = _make_ring(backend)
        ...
        self.assertTrue((ring.add(a, b).astype(object) == (a_obj + b_obj) % q_col).all())
        self.assertTrue((ring.sub(a, b).astype(object) == (a_obj - b_obj) % q_col).all())
        self.assertTrue((ring.neg(a).astype(object) == (-a_obj) % q_col).all())
        ...
        self.assertTrue(np.array_equal(ring.sub(ring.add(a, b), b), a))
        self.assertTrue(np.array_equal(ring.add(a, ring.neg(a)), ring.from_signed([0] * D)))
        ...
        self.assertTrue(np.array_equal(prod, ring.from_signed([-1] + [0] * (D - 1))))
        ...
        for _ in range(5):
            ...
            self.assertTrue(np.array_equal(ring.intt(ring.ntt(vec)), vec))
        ...
        self.assertTrue((ring.mul_add(a, b, acc).astype(object) == want_mul_add).all())

        want_mul_scalar = (a_obj * s) % q_col
        self.assertTrue((ring.mul_scalar(a, s).astype(object) == want_mul_scalar).all())

        want_mul_scalar_then_sub = (acc_obj - a_obj * s) % q_col
        self.assertTrue((ring.mul_scalar_then_sub(a, s, acc).astype(object) == want_mul_scalar_then_sub).all())
    ...
    @parameterized.parameters(
        *((backend, bad_dtype)
          ...
          for bad_dtype in (np.float64, object))
    )
    def test_non_uint64_dtype_raises_typeerror_across_the_op_surface(self, backend, bad_dtype):
