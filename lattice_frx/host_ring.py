...
class _HostRingBase:
    ...
    **Module stacks.** A module vector is a leading-axis stack of elements
    ...
    def combine(self, weights, stack: np.ndarray) -> np.ndarray:
        ...
        reach that fold. A position that is zero across the whole
        contracted axis sums to zero for every weight, so skipping it is
        exact rather than approximate, and finding those positions is one
        more pass over the same `uint64` array the contract check already
        walks. The caller this is sized for is an LNP garbage aggregation
        (eprint 2022/284, Fig. 8), which arrives with 3 of 256 matrix
        positions occupied over a 41.8 MB stack — an order of magnitude
        there. The alternative was for that consumer to scan its own
        support and hand down a narrowed stack, which every next sparse
        ...
        if weights.shape[1] != shape[0]:
            raise ValueError(
                ...
                f"for a stack of {shape[0]} terms"
            )
        ...
        # Reducing the contracted axis first; the other order gives the
        # same set and costs up to ~1.3x of a sparse call at small
        # `limbs * d`, ~1% at large.
        occupied = np.flatnonzero(flat.any(axis=0).any(axis=(-2, -1)))
        # Narrow whenever there is anything to drop, rather than at a tuned
        # occupancy: break-even measures anywhere over 80-92% depending on
        # shape, so no one constant is right, and nothing here could catch
        # one set wrong — every policy this line could hold leaves the
        # tests green, since none of them changes a result.
        taken = occupied if len(occupied) < positions else slice(None)
